...
func gfxNeedsLogin(page *rod.Page) bool {
	res, err := page.Eval(`() => {
...
func ensureGFXLogin(ctx context.Context, session *Session, startURL string) (*rod.Page, error) {
	...
	for i := 0; i < 20; i++ {
		...
		u := safeURL()
		if u != "" && !strings.Contains(u, "signin") && !gfxNeedsLogin(page) {
	...
	if gfxNeedsLogin(page) {
		hasEmail, _, _ := page.Has("input[type='email']")
		if !hasEmail {
			log.Printf("[gfx_%s] Guest on tool page — opening sign-in", accountID)
			_ = page.Timeout(30 * time.Second).Navigate("https://app.gfxtoolz.ai/signin")
			for i := 0; i < 25; i++ {
				if ok, _, _ := page.Has("input[type='email']"); ok {
					break
				}
				time.Sleep(200 * time.Millisecond)
			}
	...
	for i := 0; i < 30; i++ {
		...
		urlNow := safeURL()
		if urlNow != "" && !strings.Contains(urlNow, "signin") && !gfxNeedsLogin(page) {
			isRedirected = true
			break
		}
	...
	if showDeviceLimit {
		...
		for k := 0; k < 15; k++ {
			if u := safeURL(); u != "" && !strings.Contains(u, "signin") && !gfxNeedsLogin(page) {
				isRedirected = true
				break
			}
			time.Sleep(300 * time.Millisecond)
